...
%The calculation of the efficiency uncertainty $\delta \mathcal{E}$ is an issue of special attention. Due to the fact that $N_{gen}$ and $N_{rec}$ in Eq.~\eqref{eq:eff} are not independent, the usual method of partial derivatives is not applicable in order to calculate $\delta \mathcal{E}$.
...
% For the first two $W$ bins the uncertainty from these sources are comparable, while for the other bins the model dependent uncertainty is completely dominated by the former conventional source.

%isolated from the other uncertainty sources and 
...
%the absolute uncertainty $\delta^{\text{unfold}}_{\text{model}}(\Delta X)$ associated with this correction is estimated as a half of the difference between the corrected and non-corrected cross section values (similarly to the uncertainty due to the empty cells filling). This method of accounting for this uncertainty advantages the direct comparison between the cross sections of this study and the corresponding cross sections off the free proton. 
...
%Meanwhile, the quasi-elastic scattering off the deuteron is less understood and somehow lacks the quality of theoretical description compared to the elastic scattering off the free proton. In Sect.~\ref{Sect:norm} the ability of the Bosted parameterization to describe available experimental data is tested, and then the comparison between the cross section estimated from the current dataset and the parameterization is given.
...
The systematic effect of the FSI-background correction is estimated for each bin in $W$ and $Q^{2}$. The average value of the relative systematic uncertainty is 0.4\%, which is rather marginal.
...
%single-differential cross sections are reported with the uncertainty $\delta_{\text{stat,mod}}^{\text{tot}}(\Delta X)$ given by Eq.~\eqref{eq:error_stat_mod}. The integral cross sections are reported with the corresponding uncertainty $\delta_{\text{stat,mod}}^{\text{tot}}$, which is obtained  

%For the integral cross sections the corresponding uncertainty  is obtained from that of the single-differential cross sections according to the standard error propagation rules.
...
%As written in Sect.~\ref{Sect:corr_cr_sect} after the empty cells filling the cross sections are subject to the radiative correction and the unfolding the effects of the target motion. The former is applied as a global factor in each $\Delta W \Delta Q^2$ bin and the uncertainty $\delta_{\text{model}} (\Delta X)$ is propagated accordingly. The latter correction is applied as a five-dimensional factor in each $\Delta W \Delta Q^2$ bin
...
%, while the single-differential cross sections are reported only with the uncertainty $\delta_{\text{stat,mod}}^{\text{tot}}$ (see Sect.~\ref{Sect:mod_dep}).

In this study the uncertainty $\delta_{\text{stat,mod}}^{\text{tot}}$ is less than the total systematic uncertainty for the majority of $(W,~Q^{2})$ points, exceeding it only near the threshold (for $W \lesssim 1.4$~GeV). This happens because the former rises close to the threshold due to small experimental statistics, large contribution of the empty cells (see Sect.~\ref{Sect:empt_cells}), and pronounced impact of the unfolding correction (see Sect.~\ref{Sect:fermi_corr}). % Meanwhile, for the majority of $W$ bins (i.e. for $W \gtrsim 1.4$~GeV ) the systematic uncertainty slightly dominates the uncertainty $\delta_{\text{stat,mod}}^{\text{tot}}$.
