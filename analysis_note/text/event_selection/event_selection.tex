...
% datasets with different configurations (hydrogen and deuterium targets as well as two different beam energies of 1 GeV and 2.039 GeV).

%All data accumulated during the run
...
Although the GSIM package includes all the detector geometry and properties, it still does not properly reproduce the resolution of the drift chambers and the TOF system. So the GSIM Post Processor (GPP) is used to better match the resolution as well as to include the effects of less-than-perfect detector response (due to broken drift wires, problematic phototubes, etc.). The latter effects are unique for a particular run period, and therefore the information on the detector imperfections is usually provided along with the data files to be then used in the GPP. Meanwhile, the GPP parameters intended to adjust the resolution (DC and TOF smearing factors) are typically determined individually during a particular analysis as the resolution depends on kinematics and hence on experimental conditions. This analysis uses the same resolution related GPP parameters as those determined in the study~\cite{Fed_an_note:2017,Fed_paper_2018}, which reports the cross section measurements off the free proton performed under the same experimental conditions (as they were also a part of the ``e1e" run period). 
 ...
 %This analysis uses the GPP parameters that were determined for the study described in Refs.~\cite{Fed_an_note:2017,Fed_paper_2018}, which reports the cross section measurements off the free proton performed under the same experimental conditions (as they were also a part of the ``e1e" run period). 
%As the resolution depends on kinematics and hence on experimental conditions, 

%he resolution depends on kinematics and hence on experimental conditions, the GPP parameters intended to adjust the resolution (DC and TOF smearing factors) are typically determined individually during a particular analysis. This analysis uses the GPP parameters that were determined for the study described in Refs.~\cite{Fed_an_note:2017,Fed_paper_2018}, which reports the cross section measurements off the free proton performed under the same experimental conditions (as they were also a part of the ``e1e" run period). 
%As the resolution depends on kinematics and hence on experimental conditions, the GPP parameters intended to adjust the resolution (DC and TOF smearing factors) are typically determined individually for a particular dataset.
%, which are unique for a particular run period.
%that, being also attributed to the``e1e" run period, were performed under the same experimental conditions.
...
%It includes several steps. Firstly, 
...
%All data accumulated during the run is stored in BOS~\cite{BOS:bank} files.

%Events are selected for analysis under the following conditions:

%\begin{itemize}
%\item the number of geometrically reconstructed particles ($gpart$) is required to be greater than zero. The $gpart$ variable is extracted from the variable $NPGP$ in the HEVT bank according to the following relation,

%\item the status word ($stat$) is greater than zero (variable $Status$ in the EVNT bank);

%\item for electron candidates all variables $DCStat$, $CCStat$, $SCStat$, and $ECStat$ from the EVNT bank must be greater than zero, while for hadron candidates only the variables $DCStat$ and $SCStat$ from the EVNT bank are required to be greater than zero; 

%\item the corresponding charge for all particle candidates (variable $Charge$ from the EVNT bank) is required to be plus or minus one depending on the particle candidate's type.
...
\end{figure}
%\vspace{-1em}
%The value of five photoelectrons was chosen as a criterion to ensure that the single-photoelectron peak itself as well as the region, where it overlaps with the main spectrum, is located below this value for most of the PMTs. This will lead to the better separation between the single-photoelectron peak and the main spectrum after the geometrical cut is applied.

%It turned out that the major part of the contamination from the single-photoelectron peak corresponds to some specific geometrical zones, which are distributed inhomogeneously in the CC plane. For the purpose of removal of these zones from the further analysis a special geometrical cut is established. 
...
%This cut leads to a significant decrease of the contamination from the single-photoelectron peak and better separation between this peak and main photoelectron distribution (that is especially essential for forward segments). 
...
%For this data analysis momentum corrections developed in the analysis of the proton part of ``e1e" dataset with the same beam energy were used~\cite{Fed_an_note:2017}. To establish them the approach~\cite{KPark:momcorr}, which is based on elastic kinematic, was chosen. These corrections include electron momentum magnitude correction as well as electron polar angle correction, which were developed for each CLAS sector individually. 
...
%Figure~\ref{fig:el_mom_corr_sim} shows the event distributions of the quantity $\Delta P$ (that is the difference between thrown and reconstructed electron momenta) before and after the correction procedure. As seen in Fig.~\ref{fig:el_mom_corr_sim}, these differences become negligible after momentum corrections have been applied.
