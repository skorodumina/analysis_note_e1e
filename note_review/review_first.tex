...
%(which includes the comparison with the corresponding free proton cross sections and investigation of FSI effects)
...
%none of the currently existing MC generators is expected to give better results for the efficiency.

% no other event generator that can simulate the Fermi smearing for the double-pion production currently exists. This EG, being especially developed for experiments off the moving proton, generates the Fermi motion of the initial proton and then naturally merges it into the specific kinematics of double-pion electroproduction. We consider the efficiency estimated by TWOPEG-D to be trustworthy relying on the following arguments.\\

% TWOPEG-D~\cite{twopeg_d} is an extension of the TWOPEG event generator~\cite{twopeg}, which simulates the double-pion production off the free proton and almost perfectly reproduces the published free proton cross sections in the investigated kinematic region as confirmed by Refs.~\cite{twopeg,fedotov_prc}. TWOPEG-D in turns assumes the quasi-free events in the initial proton rest frame to be distributed according to the free proton cross sections implemented in TWOPEG, which is considered to be a good approximation for the purpose of the efficiency evaluation.
...
%one can see also pronounced tail from pions in this region. It is clearly seen at the right plot that after removing the first CC segment the relative contribution from the pions becomes smaller for all P_{e}s.
...
%The dominant part of events in the region of $P_{e’}$$\sim$1.8 GeV is thought to correspond to the elastic part of the scattering spectrum.
...
%As events assigned to the paddle \#48 in each Sector are not trustworthy, they were not included into the analysis. 
...
%Meanwhile, for pions such a strong correlation was not observed
...
%The simulated missing mass distributions do not match the corresponding experimental distributions well enough for events from the $\pi^{-}$-missing topology and $W>$~1.4875~GeV. The difference is attributed to FSI background, which can hardly be simulated. This complicates the
